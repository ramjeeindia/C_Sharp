...
namespace SDK
{
    public class InternalSalesOrder : Main
    {
        ...
        #region Constructor
        public InternalSalesOrder() : base()  // Class Constructor
        {
        ...
        public void FormLoad()
        {
            ...
            {
                _Form.Freeze(true);
                _Form = Utilities.oApplication.SBO_Application.Forms.ActiveForm;
                //_dbDataSourceMaster = Form.DataSources.DBDataSources.Item("@TA_OSTUC");
                //SetValue();
                //_Form.EnableMenu("4870", true);
                //_Form.EnableMenu("4869", true);
                _Form.Freeze(false);
            }
            catch (Exception Ex)
            {
                Utilities.ShowErrorMessage(Ex.Message.ToString());
                _Form.Freeze(false);
            }
        }
                ...
                if (pVal.BeforeAction == true)
                {
                    switch (pVal.MenuUID)
                    {
                        case Constants.User_Menus.MENU_InternalSalesOrder:
                            Utilities.oApplication.SBO_Application.MessageBox("You are in Internal Sales Order Program");
